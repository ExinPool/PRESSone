...
``` bash
# PRESSone node blocks monitor
* * * * * cd /data/monitor/exinpool/PRESSone/monitor && bash prs_blocks.sh >> prs_blocks.log &
```

The crontab will run every minute then you can check the log in the `prs_blocks.log`.
